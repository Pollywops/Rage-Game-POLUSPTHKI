...
textures = [os.path.join("textures/", d) for d in [f for f in os.listdir('textures') if f.lower().endswith('.png')]]
...
class Blocks(pygame.sprite.Sprite):
    ...
    def update(self,offesetx,offsety):
...
def build_blocks_from_matrix(matrix):
    ...
    for y,row in enumerate(matrix):
        for x,val in enumerate(row):
            if val != " ":
                b = Blocks(x*grid_size,y*grid_size,x,y,grid_size,grid_size,textures[int(val)],int(val),GREEN)
                blocks.add(b)
                blockorder.append(b)
...
loaded = load_from_json("level.json")
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_1:
                if selected_block <= len(textures)-1:
                    selected_block +=1
                else:
                    selected_block = 0
